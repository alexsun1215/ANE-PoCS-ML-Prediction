...
data.drop(columns=['death','postprocedural_stroke'],axis=1,inplace=True)
...
select=RFE(RFC_, n_features_to_select=35, step=1).fit(X_resampled, y_resampled)
...
from sklearn.linear_model import LogisticRegression
clf=LogisticRegression(random_state=1,C=199.7670704)
clf.fit(X_train,y_train)
threshold=0.5
# y_pred_train1=clf.predict(X_train)
y_pred_train2=clf.predict_proba(X_train)[:,1]
...
import lightgbm as lgb
threshold=0.48
clf=lgb.LGBMClassifier(random_state=1,max_depth=2,n_estimators=49)
clf.fit(X_train,y_train)
# y_pred_train1=clf.predict(X_train)
y_pred_train2=clf.predict_proba(X_train)[:,1]
y_pred_train1=[int(i>threshold) for i in y_pred_train2]
...
import lightgbm as lgb
clf=cgb.CatBoostClassifier(random_seed=1,depth=1,iterations=96,verbose=0)
clf.fit(X_train,y_train)
threshold=0.5
# y_pred_train1=clf.predict(X_train)
y_pred_train2=clf.predict_proba(X_train)[:,1]
...
test_acc_list_cgb,test_Precision_list_cgb,test_recall_list_cgb,test_f1_list_cgb,test_auc_list_cgb=model_cv_score1(clf,X_resampled[lasso_svm_selcet],y_resampled,a=0.5)
...
def clfResult(vardim, x, bound):
    ...
    print("ACC = ",metrics.roc_auc_score(y_test,predictval)) # R2
    return metrics.roc_auc_score(y_test,predictval)
bound = (np.array([[1,1,0.0001],[2,20,0.001]]))
...
clf1 = LogisticRegression(random_state=1,C=0.00053522)
clf2 =cgb.CatBoostClassifier(random_seed=1,depth=2,iterations=18,verbose=0)
estimators = [
('LR',     clf1),
('Catboost',    clf2),
...
def clfResult(vardim, x, bound):
    ...
    print("ACC = ",metrics.roc_auc_score(y_test,predictval)) # R2
    return metrics.roc_auc_score(y_test,predictval)
bound = (np.array([[1,1,0.01],[2,100,100]]))
ga = GeneticAlgorithm(19, 3, bound, 2, [0.75, 0.25, 0.5])
ga.solve()
...
clf1 = LogisticRegression(random_state=1,C=35.58484335)
clf2 =lgb.LGBMClassifier(random_seed=1,max_depth=2,n_estimators=98,verbose=0)
estimators = [
('LR',     clf1),
('lgb',    clf2),
...
model_cv_score1(clf,X_resampled[lasso_svm_selcet],y_resampled,random_state=22,a=0.5)
...
def clfResult(vardim, x, bound):
    ...
    print("ACC = ",metrics.roc_auc_score(y_test,predictval)) # R2
    return metrics.roc_auc_score(y_test,predictval)
bound = (np.array([[1,1,0.01,1,1],[2,30,1,2,30]]))
...
clf1 = LogisticRegression(random_state=1,C=0.09600586   )
clf2 =lgb.LGBMClassifier(random_seed=1,max_depth=1,n_estimators=12,verbose=0)
clf3 =cgb.CatBoostClassifier(random_seed=1,depth=2,iterations=28,verbose=0)
estimators = [
('LR',     clf1),
('lgb',    clf2),
('catboost',    clf3),
