...
import lightgbm as lgb
clf=cgb.CatBoostClassifier(random_seed=1,depth=1,iterations=62,verbose=0)
clf.fit(X_train,y_train)
threshold=0.5
# y_pred_train1=clf.predict(X_train)
y_pred_train2=clf.predict_proba(X_train)[:,1]
...
def clfResult(vardim, x, bound):
    ...
    print("ACC = ",metrics.roc_auc_score(y_test,predictval)) # R2
    return metrics.roc_auc_score(y_test,predictval)
bound = (np.array([[1,1,0.0001],[2,30,0.001]]))
...
clf1 = LogisticRegression(random_state=1,C=0.00082457)
clf2 =cgb.CatBoostClassifier(random_seed=1,depth=2,iterations=28,verbose=0)
estimators = [
('LR',     clf1),
('Catboost',    clf2),
...
clf1 = LogisticRegression(random_state=1,C=83.94297158)
clf2 =lgb.LGBMClassifier(random_seed=1,max_depth=2,n_estimators=92,verbose=0)
estimators = [
('LR',     clf1),
('lgb',    clf2),
...
def clfResult(vardim, x, bound):
    ...
    print("ACC = ",metrics.roc_auc_score(y_test,predictval)) # R2
    return metrics.roc_auc_score(y_test,predictval)
bound = (np.array([[1,1,0.01,1,1],[2,50,1,2,50]]))
...
clf1 = LogisticRegression(random_state=1,C=0.86359374   )
clf2 =lgb.LGBMClassifier(random_seed=1,max_depth=2,n_estimators=4,verbose=0)
clf3 =cgb.CatBoostClassifier(random_seed=1,depth=2,iterations=49,verbose=0)
estimators = [
('LR',     clf1),
('lgb',    clf2),
('catboost',    clf3),
