...
data=pd.read_excel('数据简化特征版本.xlsx',sheet_name='death')
...
from sklearn.linear_model import LogisticRegression
clf=LogisticRegression(random_state=1,C=0.00009564)
clf.fit(X_train,y_train)
threshold=0.5
# y_pred_train1=clf.predict(X_train)
y_pred_train2=clf.predict_proba(X_train)[:,1]
...
import lightgbm as lgb
threshold=0.48
clf=lgb.LGBMClassifier(random_state=1,max_depth=1,n_estimators=14)
clf.fit(X_train,y_train)
# y_pred_train1=clf.predict(X_train)
y_pred_train2=clf.predict_proba(X_train)[:,1]
y_pred_train1=[int(i>threshold) for i in y_pred_train2]
...
def clfResult(vardim, x, bound):
    ...
    print("ACC = ",metrics.roc_auc_score(y_test,predictval)) # R2
    return metrics.roc_auc_score(y_test,predictval)
bound = (np.array([[1,1,0.0001],[1,5,0.001]]))
...
clf1 = LogisticRegression(random_state=1,C=0.00055956)
clf2 =cgb.CatBoostClassifier(random_seed=1,depth=1,iterations=5,verbose=0)
estimators = [
('LR',     clf1),
('Catboost',    clf2),
...
def clfResult(vardim, x, bound):
    ...
    print("ACC = ",metrics.roc_auc_score(y_test,predictval)) # R2
    return metrics.roc_auc_score(y_test,predictval)
bound = (np.array([[1,1,0.0001],[1,14,0.01]]))
ga = GeneticAlgorithm(19, 3, bound, 2, [0.75, 0.25, 0.5])
ga.solve()
...
clf1 = LogisticRegression(random_state=1,C=0.00941973)
clf2 =lgb.LGBMClassifier(random_seed=1,max_depth=1,n_estimators=13,verbose=0)
estimators = [
('LR',     clf1),
('lgb',    clf2),
...
def clfResult(vardim, x, bound):
    ...
    print("ACC = ",metrics.roc_auc_score(y_test,predictval)) # R2
    return metrics.roc_auc_score(y_test,predictval)
bound = (np.array([[1,1,0.01,1,1],[1,15,1,1,20]]))
...
from sklearn.ensemble import StackingClassifier
clf1 = LogisticRegression(random_state=1,C=0.28447507      )
clf2 =lgb.LGBMClassifier(random_seed=1,max_depth=1,n_estimators=9,verbose=0)
clf3 =cgb.CatBoostClassifier(random_seed=1,depth=1,iterations=20,verbose=0)
estimators = [
('LR',     clf1),
('lgb',    clf2),
('catboost',    clf3),
