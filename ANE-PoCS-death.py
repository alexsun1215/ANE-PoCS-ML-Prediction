...
data['death']=data['death'].map(lambda x: 1 if pd.isnull(x)==False else 0)
...
data.drop(columns=['postprocedural_stroke','postprocedural_atrial_fibrillation'],axis=1,inplace=True)
...
select=RFE(RFC_, n_features_to_select=31, step=1).fit(X_resampled, y_resampled)
...
lasso_svm_selcet=['congestive_heart_failure',
 'hemoglobin_Post',
 'potassium_Pre',
 'alt_Pre',
 'race',
...
clf=LogisticRegression(random_state=1,C=0.000001)
...
from sklearn.linear_model import LogisticRegression
clf=LogisticRegression(random_state=1,C=0.00009704)
clf.fit(X_train,y_train)
threshold=0.5
# y_pred_train1=clf.predict(X_train)
y_pred_train2=clf.predict_proba(X_train)[:,1]
...
test_acc_list_lr,test_Precision_list_lr,test_recall_list_lr,test_f1_list_lr,test_auc_list_lr=model_cv_score1(clf,X_resampled[lasso_svm_selcet],y_resampled,a=0.45)
...
threshold=0.48
clf=lgb.LGBMClassifier(random_state=1,max_depth=1,n_estimators=10)
clf.fit(X_train,y_train)
# y_pred_train1=clf.predict(X_train)
y_pred_train2=clf.predict_proba(X_train)[:,1]
y_pred_train1=[int(i>threshold) for i in y_pred_train2]
...
import lightgbm as lgb
threshold=0.48
clf=lgb.LGBMClassifier(random_state=1,max_depth=1,n_estimators=13)
clf.fit(X_train,y_train)
# y_pred_train1=clf.predict(X_train)
y_pred_train2=clf.predict_proba(X_train)[:,1]
y_pred_train1=[int(i>threshold) for i in y_pred_train2]
...
test_acc_list_lgb,test_Precision_list_lgb,test_recall_list_lgb,test_f1_list_lgb,test_auc_list_lgb=model_cv_score1(clf,X_resampled[lasso_svm_selcet],y_resampled,a=0.45)
...
clf=cgb.CatBoostClassifier(random_seed=1,depth=1,iterations=4,verbose=0)
clf.fit(X_train,y_train)
threshold=0.45
# y_pred_train1=clf.predict(X_train)
y_pred_train2=clf.predict_proba(X_train)[:,1]
...
clf1 = LogisticRegression(random_state=1,C=0.00066328)
...
clf1 = LogisticRegression(random_state=1,C=0.00996999)
clf2 =lgb.LGBMClassifier(random_seed=1,max_depth=1,n_estimators=5,verbose=0)
estimators = [
('LR',     clf1),
('lgb',    clf2),
...
def clfResult(vardim, x, bound):
    ...
    print("ACC = ",metrics.roc_auc_score(y_test,predictval)) # R2
    return metrics.roc_auc_score(y_test,predictval)
bound = (np.array([[1,1,0.01,1,1],[1,10,1,1,10]]))
...
from sklearn.ensemble import StackingClassifier
clf1 = LogisticRegression(random_state=1,C=0.40868251 )
clf2 =lgb.LGBMClassifier(random_seed=1,max_depth=1,n_estimators=4,verbose=0)
clf3 =cgb.CatBoostClassifier(random_seed=1,depth=1,iterations=9,verbose=0)
estimators = [
('LR',     clf1),
('lgb',    clf2),
('catboost',    clf3),
...
model_cv_score1(clf,X_resampled[lasso_svm_selcet],y_resampled,random_state=1,a=0.5)
